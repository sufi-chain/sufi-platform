...
public class HooshvareRequestContextMetadataTests
{
    [Theory]
    ...
    public void Should_Preserve_Unicode_In_Built_And_Merged_Metadata(string text)
    {
        ...
        var built = HooshvareRequestContextMetadata.BuildJson(context);
        var merged = HooshvareRequestContextMetadata.Merge("{\"kind\":\"voice\"}", context);
        var combined = HooshvareRequestContextMetadata.MergeMetadata(merged, built)!;

        foreach (var metadata in new[] { built, merged, combined })
        {
            metadata.ShouldContain(text);
            metadata.ShouldNotContain("\\u");
            HooshvareRequestContextMetadata.TryParseContext(metadata)["title"].ShouldBe(text);
        }
    ...
    public void Should_Roundtrip_Json_Quotes_Backslashes_And_Newlines()
    {
        ...
        var metadata = HooshvareRequestContextMetadata.BuildJson(
    ...
    {
        HooshvareRequestContextMetadata.TryParseContext(
            "{\"hooshvareContext\":{\"title\":\"\\u0622\"}}")
            ["title"].ShouldBe("آ");
    }
}
