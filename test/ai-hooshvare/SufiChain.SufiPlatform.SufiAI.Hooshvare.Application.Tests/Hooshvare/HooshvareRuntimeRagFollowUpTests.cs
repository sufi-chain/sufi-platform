...
public partial class HooshvareRuntimeRagWorkspaceTests
{
    ...
    public async Task Should_Keep_Raw_Hits_Without_Contextual_Retry()
    {
        ...
        result.RagSearch.QueryMode.ShouldBe(HooshvareRagQueryMode.Raw);
        result.RagSearch.Outcome.ShouldBe(HooshvareRagSearchOutcome.Retrieved);
    ...
    public async Task Should_Retry_Empty_FollowUp_Once_Within_Original_Project_Scope(string message)
    {
        ...
        result.RagSearch.QueryMode.ShouldBe(HooshvareRagQueryMode.ContextualRetry);
        result.RagSearch.Outcome.ShouldBe(HooshvareRagSearchOutcome.Retrieved);
    ...
    public async Task Should_Still_Search_Social_Or_New_Topic_Turns_Without_Retry(string message)
    {
        ...
        result.RagSearch.AttemptCount.ShouldBe(1);
        result.RagSearch.QueryMode.ShouldBe(HooshvareRagQueryMode.Raw);
        result.RagSearch.Outcome.ShouldBe(HooshvareRagSearchOutcome.NoMatches);
    ...
    public async Task Should_Expand_With_Successful_Query_And_Preserve_All_Metadata_Filters()
    {
        ...
        var expectedQuery = HooshvareRagFollowUpQueryBuilder.TryBuild(input);
        fixture.Rag.SearchAsync(Arg.Any<SufiAIRagSearchRequest>(), Arg.Any<CancellationToken>())
            .Returns(call =>
            {
                var request = call.Arg<SufiAIRagSearchRequest>();
                ...
                if (request.MetadataFilters.ContainsKey("articleId"))
                {
                    ...
                    request.MinSimilarity.ShouldBe(HooshvareRagRuntimeOptionsDefaults.ArticleExpandMinSimilarity);
                }
    ...
    public async Task Should_Not_Retry_When_Cancelled_After_Empty_Raw_Search()
    {
        ...
        await fixture.ProgressReporter.DidNotReceive().ReportAsync(
            Arg.Is<HooshvareTurnProgressDto>(progress => progress.Stage == HooshvareTurnProgressStages.SearchingKb && progress.Status == HooshvareTurnProgressStatuses.Succeeded),
    ...
    public async Task Should_Report_NotAttempted_When_Rag_Is_Disabled()
    {
        var projectId = Guid.NewGuid();
        var fixture = CreateFixture(projectId, [projectId], IndexingWorkspaceName);
        fixture.Definition.SetRuntimeOptions(new HooshvareRuntimeOptions { UseRag = false });

        var result = await fixture.Orchestrator.PrepareRequestAsync(fixture.Definition, CreateFollowUpInput(fixture, "لایسنسش چیه؟"));

        result.RagSearch.Outcome.ShouldBe(HooshvareRagSearchOutcome.NotAttempted);
    ...
    private static HooshvareRuntimeRequestDto CreateFollowUpInput(Fixture fixture, string message)
    {
    ...
    private static async Task AssertSingleSearchProgressAsync(Fixture fixture, string detail)
    {
        await fixture.ProgressReporter.Received(1).ReportAsync(
            ...
            Arg.Any<CancellationToken>());
        await fixture.ProgressReporter.Received(1).ReportAsync(
            Arg.Is<HooshvareTurnProgressDto>(progress => progress.Stage == HooshvareTurnProgressStages.SearchingKb && progress.Status == HooshvareTurnProgressStatuses.Succeeded && progress.Detail == detail),
            Arg.Any<CancellationToken>());
        await fixture.ProgressReporter.DidNotReceive().ReportAsync(
            Arg.Is<HooshvareTurnProgressDto>(progress => progress.Stage == HooshvareTurnProgressStages.SearchingKb && progress.Status == HooshvareTurnProgressStatuses.Failed),
            Arg.Any<CancellationToken>());
