...
public class CopilotMcpProgressFunctionFilterTests
{
    [Theory]
    ...
    public async Task Registered_Tool_Progress_Should_Resolve_Specific_Activity_In_Every_Culture(string toolName)
    {
        ...
        progress[0].Stage.ShouldBe(CopilotTurnProgressStages.CallingTool);
        progress[0].Status.ShouldBe(CopilotTurnProgressStatuses.Started);
        progress[1].Stage.ShouldBe(CopilotTurnProgressStages.ToolResult);
        progress[1].Status.ShouldBe(CopilotTurnProgressStatuses.Succeeded);
        foreach (var step in progress)
        {
            step.ToolName.ShouldBe(toolName);
            foreach (var culture in new[] { "en", "fa", "ar", "es" })
            {
                using var stream = typeof(CopilotMcpProgressFunctionFilterTests).Assembly
    ...
    public async Task Failed_Tool_Progress_Should_Keep_Original_Name()
    {
        ...
        progress.Count.ShouldBe(2);
        progress[1].Stage.ShouldBe(CopilotTurnProgressStages.ToolResult);
        progress[1].Status.ShouldBe(CopilotTurnProgressStatuses.Failed);
    ...
    private static async Task<(Kernel Kernel, KernelFunction Function, List<CopilotTurnProgressDto> Progress)>
    ...
    private static List<CopilotTurnProgressDto> AttachReporter(Kernel kernel)
    {
        var progress = new List<CopilotTurnProgressDto>();
        var reporter = Substitute.For<ICopilotTurnProgressReporter>();
        reporter.ReportAsync(Arg.Any<CopilotTurnProgressDto>(), Arg.Any<CancellationToken>())
            .Returns(call =>
            {
                progress.Add(call.Arg<CopilotTurnProgressDto>());
                return Task.CompletedTask;
            });
        kernel.FunctionInvocationFilters.Add(new CopilotMcpProgressFunctionFilter(reporter));
        return progress;
    }
}
