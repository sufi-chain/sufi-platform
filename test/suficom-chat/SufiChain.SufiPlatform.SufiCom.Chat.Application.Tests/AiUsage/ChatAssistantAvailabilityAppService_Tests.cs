...
public class ChatAssistantAvailabilityAppService_Tests : ChatApplicationTestBase<SufiComChatApplicationTestModule>
{
    ...
    private readonly ICopilotCatalogAppService _copilotCatalogAppService;
    ...
    public ChatAssistantAvailabilityAppService_Tests()
    {
        ...
        _copilotCatalogAppService = GetRequiredService<ICopilotCatalogAppService>();
    }
    ...
    public async Task Should_Be_Available_When_All_Checks_Pass()
    {
        ...
        var publicAssistants = await _copilotCatalogAppService.GetListAsync(new GetCopilotCatalogInput
        {
    ...
    public async Task Should_Return_Only_Default_Inbox_Assistant_When_Allowlist_Is_Default()
    {
        ...
        var publicAssistant = publicAssistants.FirstOrDefault(item =>
            string.Equals(item.Key, ChatCopilotKeys.PublicAssistant.Key, StringComparison.Ordinal));
    ...
    public async Task Should_Be_Unavailable_When_Allowlist_Has_No_Healthy_Matches()
    {
        ...
        availability.ReasonCode.ShouldBe("NoPublicCopilots");
    ...
    [Fact]
    public async Task Should_Be_Unavailable_When_No_Public_Copilot_Has_Healthy_Workspace()
    {
        await ConfigureAvailableAssistantAsync();
    ...
    private async Task ConfigureAvailableAssistantAsync()
    {
        ...
        try
        {
            var copilot = await _copilotCatalogAppService.GetByKeyAsync(ChatCopilotKeys.PublicAssistant.Key);
            _workspaceProvider.HealthyWorkspaceIds.Add(copilot.WorkspaceId);
            if (!string.IsNullOrWhiteSpace(copilot.WorkspaceName))
            {
                _workspaceProvider.HealthyWorkspaces.Add(copilot.WorkspaceName);
            }
        }
        catch
        {
            // Copilot catalog may be unavailable in isolated test setups.
        }
    }
