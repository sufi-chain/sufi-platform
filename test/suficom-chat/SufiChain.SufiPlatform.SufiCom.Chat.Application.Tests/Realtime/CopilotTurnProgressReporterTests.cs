...
public class CopilotTurnProgressReporterTests
{
    [Fact]
    public async Task Transient_turn_routes_to_owner_and_restores_the_chat_scope()
    {
        var accessor = new CopilotTurnProgressAccessor();
        var notifier = Substitute.For<IChatRealtimeNotifier>();
        var clock = Substitute.For<IClock>();
        var reporter = new ChatCopilotTurnProgressReporter(accessor, notifier, clock);
        var sessionId = Guid.NewGuid();
        var tenantId = Guid.NewGuid();
        var turnId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var progress = new CopilotTurnProgressDto
        {
            Stage = CopilotTurnProgressStages.CallingTool,
        ...
        await notifier.Received(1).NotifyCopilotTurnProgressAsync(userId,
