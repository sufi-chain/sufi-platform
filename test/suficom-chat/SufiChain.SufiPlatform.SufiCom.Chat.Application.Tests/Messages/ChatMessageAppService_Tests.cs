...
public class ChatMessageAppService_Tests : ChatApplicationTestBase<SufiComChatApplicationTestModule>
{
    ...
    [Fact]
    public async Task Should_Reject_Authenticated_Sender_User_Id_Spoofing()
    {
        var session = await CreateAuthenticatedSessionAsync();
        ...
        using (CurrentUser.Change(ChatTestData.UserAId))
        {
            var exception = await Should.ThrowAsync<Volo.Abp.BusinessException>(async () =>
            {
                await _messageAppService.SendAsync(new SendChatMessageInput
                {
                    ...
                    SenderKind = ChatMessageSenderKind.Visitor,
                    SenderUserId = ChatTestData.UserBId,
                    AccessMode = AccessMode.PublicAuthenticated
                });
            });

            exception.Code.ShouldBe(ChatErrorCodes.InvalidParticipant);
        }
    }
    ...
    [Fact]
    public async Task Should_Clear_Internal_Messages_When_Clearing_Entire_Session()
    {
        var session = await CreateAuthenticatedSessionAsync();
        ...
        await _messageAppService.ClearSessionMessagesAsync(session.Id);

        (await _messageRepository.GetCountBySessionAsync(session.Id, includeInternal: true))
            .ShouldBe(0);
    }
