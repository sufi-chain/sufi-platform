...
public static class ChatTestServiceConfiguration
{
    public static void ConfigureTestServices(ServiceConfigurationContext context)
    {
        ...
        context.Services.AddSingleton<TestCopilotCatalogAppService>();
        context.Services.Replace(ServiceDescriptor.Singleton<ICopilotCatalogAppService>(sp =>
            sp.GetRequiredService<TestCopilotCatalogAppService>()));
