...
using SufiChain.SufiPlatform.SufiCom.Chat.ConversationLinks;
...
public class ChatTenantIsolation_Tests : ChatApplicationTestBase<SufiComChatApplicationTestModule>
{
    ...
    public async Task Conversation_Links_Should_Be_Tenant_Scoped()
    {
        ...
        using (CurrentTenant.Change(ChatTestData.TenantAId))
        {
            ...
            var link = await _conversationLinkAppService.CreateAsync(new CreateConversationLinkInput
            {
                ...
                LinkedEntityType = "SufiHelpDesk.Ticket",
                LinkedEntityId = Guid.NewGuid().ToString("D"),
                ExtraProperties =
                {
                    ["test.source"] = "tenant-isolation"
                }
            });

            tenantALinkId = link.Id;
            link.ExtraProperties["test.source"].ShouldBe("tenant-isolation");
        }
