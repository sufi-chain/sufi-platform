...
public class HooshvareTurnHubTests
{
    [Fact]
    public async Task Joining_and_publishing_use_the_same_authenticated_owner_group()
    {
        ...
        var group = ChatRealtimeGroups.HooshvareTurn(turnId, userId, tenantId);
        await hub.JoinHooshvareTurnGroupAsync(turnId);
        ...
        await notifier.NotifyHooshvareTurnProgressAsync(userId, progress);
        await client.Received(1).AssistantProgress(progress);
        await hub.LeaveHooshvareTurnGroupAsync(turnId);
        await hub.Groups.Received(1).RemoveFromGroupAsync("connection", group, Arg.Any<CancellationToken>());

        ChatRealtimeGroups.HooshvareTurn(turnId, Guid.NewGuid(), tenantId).ShouldNotBe(group);
        ChatRealtimeGroups.HooshvareTurn(turnId, userId, Guid.NewGuid()).ShouldNotBe(group);
        ChatRealtimeGroups.HooshvareTurn(turnId, userId, null).ShouldNotBe(group);
    }
