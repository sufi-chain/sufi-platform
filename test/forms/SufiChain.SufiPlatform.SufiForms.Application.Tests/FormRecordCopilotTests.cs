...
public class FormRecordCopilotTests
{
    [Theory]
    [InlineData(SufiFormsCopilotKeys.RecordAnalyst, true, null, true)]
    [InlineData(SufiFormsCopilotKeys.RecordAssistant, true, null, true)]
    [InlineData(SufiFormsCopilotKeys.RecordAssistant, false, null, false)]
    [InlineData(SufiFormsCopilotKeys.RecordAnalyst, true, false, false)]
    [InlineData(SufiFormsCopilotKeys.RecordAnalyst, false, true, true)]
        ...
        string key, bool configured, bool? selected, bool expected)
    {
        ...
        input.CopilotKey = key;
        input.PersistChatSession = selected;
        var reply = await service.SendAsync(input);

        await service.RuntimeMock.Received(1).SendAsync(Arg.Is<CopilotRuntimeRequestDto>(request =>
    ...
    [Fact]
    public async Task Another_copilot_or_an_unowned_session_cannot_be_used()
    {
        var service = new Records();
    ...
    [Fact]
    public async Task Unrelated_copilots_are_rejected_before_catalog_or_runtime_access()
    {
        var service = new Records();
        var input = service.Input();
        input.CopilotKey = SufiFormsCopilotKeys.FormCreator;
    ...
    public async Task Assistant_context_keeps_confirmation_record_schema_and_selected_model(bool confirmed)
    {
        var service = new Records();
        var input = service.Input();
        input.CopilotKey = SufiFormsCopilotKeys.RecordAssistant;
        ...
        CopilotRuntimeRequestDto? sent = null;
        service.RuntimeMock.SendAsync(Arg.Do<CopilotRuntimeRequestDto>(value => sent = value), default)
            .Returns(new CopilotRuntimeResultDto { SessionId = service.Session.Id });
        await service.SendAsync(input);
        sent.ShouldNotBeNull();
        sent.ModelConfigurationId.ShouldBe(input.ModelConfigurationId);
        using var metadata = JsonDocument.Parse(sent.MetadataJson!);
        var context = metadata.RootElement.GetProperty("copilotContext");
    ...
    {
        var fileId = Guid.NewGuid();
        var json = JsonSerializer.Serialize(new CopilotRuntimeRequestDto { AttachmentFileIds = [fileId] });
        json.ShouldNotContain(nameof(CopilotRuntimeRequestDto.AttachmentFileIds));
        JsonSerializer.Deserialize<CopilotRuntimeRequestDto>(
            "{\"AttachmentFileIds\":[\"" + fileId + "\"]}")!.AttachmentFileIds.ShouldBeEmpty();
    }

    private sealed class Records : FormRecordCopilotAppService
    {
        public ICopilotRuntimeAppService RuntimeMock { get; } = Substitute.For<ICopilotRuntimeAppService>();
        ...
        public CopilotCatalogItemDto Settings { get; } = new()
        {
        ...
        public Records()
        {
            ...
            RuntimeMock.SendAsync(Arg.Any<CopilotRuntimeRequestDto>(), default)
                .Returns(new CopilotRuntimeResultDto { Message = "Ready", SessionId = Session.Id });
        ...
        public FormRecordCopilotInput Input() => new()
        {
            Message = "Find records", CopilotKey = SufiFormsCopilotKeys.RecordAnalyst,
            FormKey = "orders", SessionId = Session.Id
        };

        protected override ICopilotRuntimeAppService Runtime => RuntimeMock;
