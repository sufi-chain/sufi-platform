...
public class FormCreatorProgressTests
{
    ...
    public async Task Subscription_joins_before_returning_filters_events_and_rejoins_until_disposed()
    {
        ...
        hub.JoinHooshvareTurnAsync(Arg.Any<Guid>(), Arg.Any<CancellationToken>()).Returns(joined.Task);
        ...
        await hub.Received(2).JoinHooshvareTurnAsync(subscription.TurnId, Arg.Any<CancellationToken>());
        await subscription.DisposeAsync();
        await reconnect("late-reconnect");
        await receive(new() { TurnId = subscription.TurnId });
        received.Count.ShouldBe(1);
        await hub.Received(2).JoinHooshvareTurnAsync(subscription.TurnId, Arg.Any<CancellationToken>());
        await hub.Received(1).LeaveHooshvareTurnAsync(subscription.TurnId, Arg.Any<CancellationToken>());
    ...
    {
        var hub = Substitute.For<IChatHubClientService>();
        hub.JoinHooshvareTurnAsync(Arg.Any<Guid>(), Arg.Any<CancellationToken>())
    ...
    public async Task Disposal_waits_for_inflight_rejoin_then_leaves_even_when_cleanup_fails()
    {
        ...
        hub.JoinHooshvareTurnAsync(subscription.TurnId, Arg.Any<CancellationToken>()).Returns(rejoining.Task);
        hub.LeaveHooshvareTurnAsync(subscription.TurnId, Arg.Any<CancellationToken>())
