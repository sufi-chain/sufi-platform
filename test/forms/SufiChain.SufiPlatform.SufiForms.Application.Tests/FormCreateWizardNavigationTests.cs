...

{
    ...
    public async Task Rich_tools_follow_hooshvare_flags_and_platform_capabilities(bool audio, bool attachments, bool platformEnabled)
    ...
    [InlineData("AIHooshvare:HooshvareNotFound")]

    [InlineData("AIHooshvare:HooshvareDisabled")]

    public async Task Unavailable_hooshvare_finishes_loading_without_blocking_manual_creation(string code)

    {

        var catalog = Substitute.For<IHooshvareCatalogAppService>();

        catalog.GetByKeyAsync(SufiFormsHooshvareKeys.FormCreator)

            .Returns(Task.FromException<HooshvareCatalogItemDto>(new Volo.Abp.BusinessException(code)));
    ...
    public async Task Missing_hooshvare_permission_allows_manual_creation()

    {
        ...
        catalog.GetByKeyAsync(SufiFormsHooshvareKeys.FormCreator)
            ...
            .Returns(Task.FromException<HooshvareCatalogItemDto>(new Volo.Abp.Authorization.AbpAuthorizationException()));
    ...
    public async Task Opening_loads_current_hooshvare_options_and_reopening_resets_model_selection()

    {

        var catalog = Substitute.For<IHooshvareCatalogAppService>();

        var settings = new HooshvareCatalogItemDto
        ...
        await catalog.Received(2).GetByKeyAsync(SufiFormsHooshvareKeys.FormCreator);
...

    {
        ...
        public TestWizard(IFormCreatorAppService service, IHooshvareCatalogAppService? catalog = null,
        ...
        public HooshvareCatalogItemDto? Settings => Hooshvare;
