...
Add pages in your module assembly; inherit `AIManagementComponentBase` only if referencing the Blazor package is acceptable, otherwise use your module base.
...
```csharp
var aiMenu = context.Menu.FindMenuItem(AIManagementMenus.GroupName);
...
Subscribe to AI-related ETOs from your module (e.g. HelpDesk `ArticlePublishedEto` → trigger `StartIndexingAsync`). Handlers may live in the HelpDesk host or a thin adapter in AI Management—prefer HelpDesk-owned handlers to avoid coupling AIManagement to HelpDesk types.

## Best practices

- Virtual methods on app services in reusable modules
- Localize user-facing strings
- Test indexing and search in AIManagement UI before product launch
