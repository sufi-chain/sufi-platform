...
Application services implementing `IApplicationService` are exposed as REST endpoints by ABP conventions (area `ai-management`, route names derived from service/method names). Use HTTP API client proxies or OpenAPI from the host.

### OpenAICompatibleController

Explicit OpenAI-style routes (see `SufiChain.SufiAbp.AIManagement.HttpApi`):
...
Authorization uses `AIManagementPermissions.Workspaces.Default` on chat completions. Pass the configured workspace **name** (not GUID) on requests.
