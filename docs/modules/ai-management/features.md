...
- **Route:** `/admin/ai-management/workspaces`
...
- **Model Configurations** (`/admin/ai-management/model-configurations`)
...
- **Test Chat** (`/admin/ai-management/test-chat`) — conversational UI with optional streaming, token/latency display
- **Multi-Modal Test** (`/admin/ai-management/multimodal-test`) — tabs for chat, audio transcription, TTS, vision, embeddings (tabs respect capability permissions)

### Analytics

- **Usage Analytics** (`/admin/ai-management/usage-analytics`) — cost, tokens, requests, success rate, breakdowns, recent logs, date range filter

### RAG

- **RAG Search** (`/admin/ai-management/rag`) — semantic search over indexed document sources for a workspace
- **Indexing Status** (`/admin/ai-management/indexing-status`) — list sources, start/re-index, monitor progress

### MCP

- **MCP Tools** (`/admin/ai-management/mcp-tools`) — browse tools, view JSON schema (execution via API)
- **MCP Servers** (`/admin/ai-management/mcp-servers`) — CRUD, enable/disable, **test connection** per server
...
When File-Manager is installed and `AIManagementOptions.SeedFileStructure` is enabled, the module seeds an **AIManagement** file structure (images, audio, video, documents) for workspace uploads. Without File-Manager, blob storage fallback applies.
