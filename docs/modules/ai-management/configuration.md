...
## Module options (`AIManagementOptions`)
...
Configure<FileManagerOptions>(fileManagerOptions =>
{
    var aiOptions = context.Services
        ...
        .GetRequiredService<IOptions<AIManagementOptions>>().Value;
...
|----------|---------|-------------|
| `SeedFileStructure` | `true` | Seeds the **AIManagement** file structure in File-Manager when that module is loaded |
...
[DependsOn(
    ...
    typeof(SufiAbpAIManagementEntityFrameworkCoreModule) // or MongoDB module
)]
public class YourHostModule : AbpModule { }
```

Blazor: add `SufiAbpAIManagementBlazorModule` and ensure `RouterOptions.AdditionalAssemblies` includes the Blazor assembly (module does this in `ConfigureServices`).
