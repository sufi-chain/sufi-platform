...
/// </summary>
public interface ISufiAbpAIWorkspaceCatalog
{
    /// <summary>
    /// Gets all workspaces visible to the current tenant.
    /// Returns an empty list when no provider module is installed.
    /// </summary>
    Task<List<SufiAbpAIWorkspaceDescriptor>> GetListAsync(CancellationToken cancellationToken = default);

    /// <summary>
    /// Finds a workspace by its unique name. Returns <c>null</c> when not found
    /// or when no provider module is installed.
    /// </summary>
    Task<SufiAbpAIWorkspaceDescriptor?> FindAsync(string name, CancellationToken cancellationToken = default);
}
