...
public class SufiAIModule : AbpModule
{
    ...
    {
        var options = context.Services.ExecutePreConfiguredActions<SufiAIWorkspaceOptions>();

        context.Services.Configure<SufiAIOptions>(workspaceOptions =>
        {
    ...
    private static void ConfigureKernel(ServiceConfigurationContext context, WorkspaceConfiguration workspaceConfig)
    {
        ...
        context.Services.AddKeyedTransient<Kernel>(
            SufiAIWorkspaceOptions.GetKernelServiceKeyName(workspaceConfig.Name),
        ...
        if (workspaceConfig.ChatClient?.Builder is null)
        {
            context.Services.AddKeyedTransient<IChatClient>(
                SufiAIWorkspaceOptions.GetChatClientServiceKeyName(workspaceConfig.Name),
                (sp, _) => sp.GetKeyedService<Kernel>(SufiAIWorkspaceOptions.GetKernelServiceKeyName(workspaceConfig.Name))?
    ...
    private static void ConfigureChatClient(ServiceConfigurationContext context, WorkspaceConfiguration workspaceConfig)
    {
        ...
        var serviceName = SufiAIWorkspaceOptions.GetChatClientServiceKeyName(workspaceConfig.Name);
