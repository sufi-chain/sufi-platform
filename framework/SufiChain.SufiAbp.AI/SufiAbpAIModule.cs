...
public class SufiAbpAIModule : AbpModule
{
    public const string DefaultWorkspaceName = "Default";

    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        var options = context.Services.ExecutePreConfiguredActions<SufiAbpAIWorkspaceOptions>();

        context.Services.Configure<SufiAbpAIOptions>(workspaceOptions =>
        {
    ...
    private static void ConfigureKernel(ServiceConfigurationContext context, WorkspaceConfiguration workspaceConfig)
    {
        ...
        context.Services.AddKeyedTransient<Kernel>(
            SufiAbpAIWorkspaceOptions.GetKernelServiceKeyName(workspaceConfig.Name),
        ...
        if (workspaceConfig.ChatClient?.Builder is null)
        {
            context.Services.AddKeyedTransient<IChatClient>(
                SufiAbpAIWorkspaceOptions.GetChatClientServiceKeyName(workspaceConfig.Name),
                (sp, _) => sp.GetKeyedService<Kernel>(SufiAbpAIWorkspaceOptions.GetKernelServiceKeyName(workspaceConfig.Name))?
    ...
    private static void ConfigureChatClient(ServiceConfigurationContext context, WorkspaceConfiguration workspaceConfig)
    {
        ...
        var serviceName = SufiAbpAIWorkspaceOptions.GetChatClientServiceKeyName(workspaceConfig.Name);
