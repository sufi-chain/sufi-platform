...
public class SufiAspNetCoreModule : AbpModule
{
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        // implement IAuditingEnabled — SufiAuditingInterceptor also checks the target type).
        context.Services.Replace(
            ServiceDescriptor.Transient<AuditingInterceptor, SufiAuditingInterceptor>());

        context.Services.Replace(
            ServiceDescriptor.Transient<Volo.Abp.AspNetCore.Auditing.AbpAuditingMiddleware, SufiAuditingMiddleware>());

        // HTTP method filter: default blacklist GET/HEAD. Hosts can override via Configure<SufiAuditingHttpMethodFilterOptions>.
        Configure<SufiAuditingHttpMethodFilterOptions>(_ => { });
        ...
        {
            options.Contributors.Add(new SufiAuditLogFallbackContributor());
            ...
            var methodFilter = new System.Lazy<SufiAuditingHttpMethodFilterOptions>(() =>
                context.Services.BuildServiceProvider().GetRequiredService<IOptions<SufiAuditingHttpMethodFilterOptions>>().Value);
