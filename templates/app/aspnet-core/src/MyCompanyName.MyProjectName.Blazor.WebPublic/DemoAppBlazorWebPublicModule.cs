...
using MyCompanyName.MyProjectName.Blazor.WebPublic.Menus;
using MyCompanyName.MyProjectName.Localization;
using MyCompanyName.MyProjectName.MultiTenancy;

namespace MyCompanyName.MyProjectName.Blazor.WebPublic;

/// <summary>
/// ABP Module for DemoApp Blazor WebPublic host.
...
public class DemoAppBlazorWebPublicModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                ...
                typeof(DemoAppBlazorWebPublicModule).Assembly
            );
        });
    }
    ...
    private void ConfigureCache()
    {
        ...
        {
            options.KeyPrefix = "DemoApp:WebPublic:";
        });
    }
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        if (hostingEnvironment.IsDevelopment())
        {
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<DemoAppBlazorWebPublicModule>(hostingEnvironment.ContentRootPath);
                // <TEMPLATE-REMOVE>
    ...
    private void ConfigureRouter(ServiceConfigurationContext context)
    {
        ...
        {
            options.AppAssembly = typeof(DemoAppBlazorWebPublicModule).Assembly;
        });
    ...
    private void ConfigureSwaggerServices(IServiceCollection services)
    {
        services.AddAbpSwaggerGen(
            options =>
            {
                options.SwaggerDoc("v1", new OpenApiInfo { Title = "DemoApp WebPublic API", Version = "v1" });
    ...
    {
        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("DemoApp_WebPublic");
        if (!hostingEnvironment.IsDevelopment())
        {
            var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]!);
            dataProtectionBuilder.PersistKeysToStackExchangeRedis(redis, "DemoApp-WebPublic-Protection-Keys");
        }
    }
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        {
            options.SwaggerEndpoint("/swagger/v1/swagger.json", "DemoApp WebPublic API");
        });
