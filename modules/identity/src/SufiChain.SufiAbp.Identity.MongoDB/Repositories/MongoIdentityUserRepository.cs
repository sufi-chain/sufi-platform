...
namespace SufiChain.SufiAbp.Identity.MongoDB.Repositories;

public class MongoIdentityUserRepository : MongoDbRepository<ISufiAbpIdentityMongoDbContext, IdentityUser, Guid>, IIdentityUserRepository
{
    public MongoIdentityUserRepository(IMongoDbContextProvider<ISufiAbpIdentityMongoDbContext> dbContextProvider)
    ...
    public virtual async Task<List<IdentityUser>> GetListByOrganizationUnitIdAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await GetUsersInOrganizationsListAsync(organizationUnitIds, cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        return await (await GetQueryableAsync(cancellationToken))
            .OrderBy(x => x.Id)
            .FirstOrDefaultAsync(u => u.UserName == userName && u.TenantId == tenantId, GetCancellationToken(cancellationToken));
    }
    ...
    public virtual async Task<long> GetCountAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        return await (await ApplyFilterAsync(
    ...
    protected virtual async Task<IQueryable<IdentityUser>> ApplyFilterAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (roleId.HasValue)
        {
            var role = await (await GetQueryableAsync<IdentityRole>(cancellationToken))
            ...
            if (role != null)
            {
                ...
                query = query.Where(u =>
                    u.Roles.Any(r => r.RoleId == roleId.Value) ||
                    u.OrganizationUnits.Any(ou => organizationUnitIds.Contains(ou.OrganizationUnitId))
                );
            }
        }
        ...
        return query
            ...
            .WhereIf(maxModificationTime != null, p => p.LastModificationTime <= maxModificationTime)
            .WhereIf(minModificationTime != null, p => p.LastModificationTime >= minModificationTime);
    }
}
