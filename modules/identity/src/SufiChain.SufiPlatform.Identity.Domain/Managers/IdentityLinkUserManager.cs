...
/// <summary>
/// Manages cross-tenant/user account links stored in the host-scoped LinkUsers table.
/// SaaS provision may call <see cref="LinkAsync"/> in-process after creating a tenant admin
/// (no app-service token required for trusted server-side linking).
/// </summary>
public class IdentityLinkUserManager : DomainService
{
    ...
    public IdentityLinkUserManager(
    ...
    protected virtual Task<List<IdentityLinkUser>> GetAllRelatedLinksAsync(
        List<IdentityLinkUser> allUsers,
        IdentityLinkUserInfo userInfo)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        using (CurrentTenant.Change(null))
        {
            if (sourceLinkUser.UserId == targetLinkUser.UserId &&
                sourceLinkUser.TenantId == targetLinkUser.TenantId)
            {
                return;
            }
    ...
    public virtual async Task<bool> IsLinkedAsync(
        IdentityLinkUserInfo sourceLinkUser,
        IdentityLinkUserInfo targetLinkUser,
        bool includeIndirect = false,
        CancellationToken cancellationToken = default)
        ...
        using (CurrentTenant.Change(null))
        {
            if (includeIndirect)
            {
                return (await GetListAsync(sourceLinkUser, true, cancellationToken: cancellationToken))
                    .Any(x =>
                        x.SourceTenantId == targetLinkUser.TenantId && x.SourceUserId == targetLinkUser.UserId ||
    ...
    public virtual async Task<bool> VerifyLinkTokenAsync(
        IdentityLinkUserInfo targetLinkUser,
        string token,
        string tokenPurpose,
        CancellationToken cancellationToken = default)
