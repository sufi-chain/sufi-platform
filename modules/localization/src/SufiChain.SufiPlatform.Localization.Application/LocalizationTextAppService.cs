...
[Authorize(LocalizationPermissions.Texts.Default)]
public class LocalizationTextAppService : SufiApplicationService, ILocalizationTextAppService
{
    ...
    public async Task<PagedResultDto<LocalizationTextDto>> GetListAsync(GetLocalizationTextsInput input)
    {
        ...
        if (hasResourceName &&
            SufiLocalizationResourceNameMap.IsReplacedAbpResource(input.ResourceName!))
        {
        ...
        {
            items = items
                .Where(item => !SufiLocalizationResourceNameMap.IsReplacedAbpResource(item.ResourceName))
                .ToList();
        }
    ...
    {
        if (SufiLocalizationResourceNameMap.IsReplacedAbpResource(resourceName))
        {
    ...
    [Authorize(LocalizationPermissions.Texts.Create)]
    public async Task<LocalizationTextDto> CreateOrUpdateAsync(CreateUpdateLocalizationTextDto input)
    {
        ...
        if (existing != null)
        {
            await CheckPolicyAsync(LocalizationPermissions.Texts.Update);
    ...
    [Authorize(LocalizationPermissions.Texts.Update)]
    ...
    [Authorize(LocalizationPermissions.Texts.Delete)]
    ...
    public async Task<List<string>> GetResourceNamesAsync()
    {
        ...
        return configuredResources
            .Union(dbResources)
            .Where(resourceName => !SufiLocalizationResourceNameMap.IsReplacedAbpResource(resourceName))
            .Distinct()
            .OrderBy(r => r)
            .ToList();
    }
    ...
    [Authorize(LocalizationPermissions.Texts.Import)]
    ...
    [Authorize(LocalizationPermissions.Texts.Export)]
