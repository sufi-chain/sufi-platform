...
[Microsoft.AspNetCore.Authorization.Authorize(SettingManagementPermissions.Emailing)]
public class EmailSettingsAppService : SettingManagementAppServiceBase, IEmailSettingsAppService
{
    ...
    [Microsoft.AspNetCore.Authorization.Authorize(SettingManagementPermissions.EmailingTest)]
    ...
    protected virtual async Task CheckFeatureAsync()
    {
        ...
        {
            throw new ApplicationException($"Feature is disabled: {SettingManagementFeatures.Enable}");
        }

        if (CurrentTenant.IsAvailable &&
            !await FeatureChecker.IsEnabledAsync(SettingManagementFeatures.AllowChangingEmailSettings))
        {
            throw new ApplicationException($"Feature is disabled: {SettingManagementFeatures.AllowChangingEmailSettings}");
        }
    }
