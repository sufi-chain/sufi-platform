...
public class AIService : DomainService, IAIService, ITransientDependency
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (workspace == null)
        {
            throw new BusinessException("AIManagement:WorkspaceNotFound")
                .WithData("WorkspaceName", workspaceName);
        }

        if (!workspace.IsActive)
        {
            throw new BusinessException("AIManagement:WorkspaceNotActive")
                .WithData("WorkspaceName", workspaceName);
        }
        ...
        if (configuration == null)
        {
            ...
            if (string.IsNullOrWhiteSpace(fallbackModelId))
            {
                throw new BusinessException("AIManagement:NoModelConfigured")
                    .WithData("WorkspaceName", workspaceName)
        ...
        if (provider == null)
        {
            throw new BusinessException("AIManagement:ProviderNotSupported")
                .WithData("Provider", workspace.Provider.ToString());
        }

        if (!provider.SupportsCapability(capabilityType))
        {
            throw new BusinessException("AIManagement:CapabilityNotSupported")
                .WithData("Provider", workspace.Provider.ToString())
                .WithData("CapabilityType", capabilityType.ToString());
        }
