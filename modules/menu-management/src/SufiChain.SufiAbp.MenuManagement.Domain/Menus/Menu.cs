...
public class Menu : FullAuditedAggregateRoot<Guid>, IMultiTenant
{
    ...
    {
        TenantId = tenantId;
        ContextType = Check.NotNullOrWhiteSpace(contextType, nameof(contextType), MenuManagementConsts.MaxContextTypeLength);
        ContextId = contextId;
        Name = Check.NotNullOrWhiteSpace(name, nameof(name), MenuManagementConsts.MaxMenuNameLength);
    ...
    {
        if (!string.IsNullOrWhiteSpace(description) && description.Length > MenuManagementConsts.MaxDescriptionLength)
        {
            throw new BusinessException(MenuManagementErrorCodes.MenuItemInvalidTarget).WithData("MaxLength", MenuManagementConsts.MaxDescriptionLength);
        }
