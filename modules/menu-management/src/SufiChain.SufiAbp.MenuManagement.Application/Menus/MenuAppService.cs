...
public class MenuAppService : SufiAbpApplicationService, IMenuAppService
{
    ...
    {
        var menu = await _menuRepository.FindByNameAsync(contextType, contextId, name, CurrentTenant.Id) ?? throw new BusinessException(MenuManagementErrorCodes.MenuNotFound).WithData("Name", name);
        return menu.ToDto();
    }
    ...
    [Authorize(MenuManagementPermissions.Menus.Edit)]
    ...
    public virtual async Task DeleteAsync(Guid id)
    {
        ...
        if (items.Count > 0) throw new BusinessException(MenuManagementErrorCodes.CannotDeleteMenuWithItems).WithData("MenuId", id);
